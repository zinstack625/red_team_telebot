...
class AdminStates(StatesGroup):
    add_creds = State()
    add_creds_json = State()


async def admin_start(msg: Message):
    keyboard = ReplyKeyboardMarkup(resize_keyboard=True)
    keyboard.add(*[KeyboardButton(option)
                   for option in ['Add creds', 'Add creds (json)', 'List and remove', 'Students']])
...
@bot.message_handler(func=lambda x: x.text == 'Add creds')
async def add_email(msg: Message):
    ...
    if admin is None:
        ...
        return
    await gather(bot.set_state(msg.from_user.id, AdminStates.add_creds),
...
@bot.message_handler(state=AdminStates.add_creds)
...
@bot.message_handler(func=lambda x: x.text == 'Add creds (json)')
async def add_email_json(msg: Message):
    admin = await db.db['Admins'].find_one({'chat_id': msg.from_user.id})
    if admin is None:
        # keep silent and act like nothing happened
        return
    await gather(bot.set_state(msg.from_user.id, AdminStates.add_creds_json),
                 bot.send_message(msg.from_user.id,
...
@bot.message_handler(state=AdminStates.add_creds_json)
async def state_add_email_json(msg: Message):
    ...
    if type(msg_dict) is dict:
        if await db.db['Emails'].find_one({'email': str(msg_dict['login'])}) is not None:
            await bot.send_message(msg.from_user.id, 'This one already exists, not adding')
            return
        await gather(give_out_email({'email': str(msg_dict['login']), 'password': str(msg_dict['password'])}),
                     bot.send_message(msg.from_user.id, 'Got it'))
    elif type(msg_dict) is list:
        tasks = []
        for pair in msg_dict:
            if await db.db['Emails'].find_one({'email': str(pair['login'])}) is not None:
                tasks.append(bot.send_message(msg.from_user.id, f'This one: {str(pair["login"])}:{str(pair["password"])}\nalready exists, not adding'))
                continue
            tasks.append(give_out_email({'email': str(pair['login']), 'password': str(pair['password'])}))
        await gather(bot.send_message(msg.from_user.id, 'Got it'),
                     *tasks)
...
async def list_and_remove(msg: Message):
    ...
    keyboard.add(*[InlineKeyboardButton(f'{doc["email"]}', callback_data=f'remove_creds_{doc["_id"]}')
                   async for doc in db.db['Emails'].find({})])
    await bot.send_message(msg.from_user.id, 'Here\'s some', reply_markup=keyboard)


@bot.callback_query_handler(func=lambda x: x.data.startswith('remove_creds_'))
